...
async function uploadPhoto(ev){
  ...
  await renderCurrentPhoto(); updateSide(); setStatus("Photo saved — detecting faces…");
...
function renderFaceEditor(){
  ...
  const options=visiblePeople().sort((a,b)=>fullName(a).localeCompare(fullName(b))).map(x=>`<option value="${x.id}" ${p&&p.id===x.id?'selected':''}>${escapeHtml(fullName(x))}${x.birth_date?' · '+escapeHtml(x.birth_date):''}</option>`).join("");
  w.innerHTML=`
    <div class="face-name-tools">
      <label>Existing person<select id="facePersonSelect"><option value="">Choose existing person…</option>${options}</select></label>
      <button class="primary full" onclick="saveFacePersonChoice()">Link selected person</button>
      <div class="divider-text">or create new person</div>
      <label>New name<input id="faceName" value="${escapeHtml(p?'':(f.label||''))}" placeholder="Full name" oninput="this.value=titleCaseName(this.value)"></label>
      <button class="full" onclick="saveFaceName()">Create and link new person</button>
      <button class="full" onclick="suggestSelectedFaceName()">Suggest correction instead</button>
    </div>`;
}

async function saveFacePersonChoice(){
  const f=faces.find(x=>x.id===selectedFaceId);
  const id=el("facePersonSelect")?.value;
  if(!f)return;
  if(!id)return alert("Choose an existing person first.");
  const p=person(id);
  if(!p)return alert("That person could not be found.");
  ...
  setStatus("Face linked to "+fullName(p));
}
