...
function preferredName(p){return p?.preferred_name || p?.given_names || fullName(p).split(' ')[0] || ''}
function nameAtPhotoDate(p,photo){
  // Phase 1 database rule: a face links to the person; names can vary over time.
  // For now, prefer a specific face label if supplied, otherwise birth_name for older photos, otherwise current display_name.
  const d=photo?.date_taken||photo?.photo_date||photo?.taken_at||'';
  if(p?.birth_name && d && p?.married_name_from && d < p.married_name_from)return p.birth_name;
  return fullName(p);
}
...
function setModeClasses(){document.body.classList.toggle("can-edit",canEdit());document.body.classList.toggle("can-delete",canDelete());document.body.classList.toggle("edit-mode",editMode||isOwner());}
...
function showPage(page){
  // Hard navigation reset: no editing screen should ever trap the user.
  document.querySelectorAll(".page").forEach(p=>p.classList.add("hidden"));
  el(page+"Page")?.classList.remove("hidden");
  document.body.dataset.page=page;
  document.querySelectorAll("nav button").forEach(b=>{
    const txt=(b.getAttribute("onclick")||"");
    b.classList.toggle("primary", txt.includes(`showPage('${page}')`) || txt.includes(`showPage(\"${page}\")`));
  });
  location.hash=page;
  if(page!=="photo"){editMode=false;setModeClasses()}
...
function leavePhoto(){editMode=false;setModeClasses();showPage('dashboard')}
function setPhotoEditMode(on){editMode=!!on;setModeClasses();setStatus(editMode?'Edit mode':'View mode')}
...
function faceElement(f){
  ...
  d.innerHTML=`<span>${escapeHtml(f.label_at_time||f.label||"")}</span><div class="handle"></div>`;
...
function renderFaceEditor(){
  ...
  if(!canEdit()){w.innerHTML=`<p><strong>${escapeHtml(p?fullName(p):(f.label||"Unnamed"))}</strong></p><textarea id="faceSuggestionText" placeholder="Suggest a correction…"></textarea><button class="primary full" onclick="suggestSelectedFaceName()">Send suggestion</button>`;return}
  w.innerHTML=`<div class="row"><label>Person</label><input id="faceName" value="${escapeHtml(p?fullName(p):(f.label||""))}" placeholder="Unnamed person" oninput="this.value=titleCaseName(this.value)"></div><div class="row"><label>Shown here</label><input id="faceLabelAtTime" value="${escapeHtml(f.label_at_time||'')}" placeholder="Name at time, e.g. Jean Davey" oninput="this.value=titleCaseName(this.value)"></div><button class="primary" onclick="saveFaceName()">Save name</button><button class="full" onclick="suggestSelectedFaceName()">Suggest correction instead</button>`;
}
...
async function saveFaceName(){
  ...
  const facePatch={person_id:p.id,label:fullName(p),status:"confirmed"};
  if('label_at_time' in f)facePatch.label_at_time=titleCaseName(el("faceLabelAtTime")?.value||'')||null;
  const upd=await sb.from("faces").update(facePatch).eq("id",f.id).select().single();
...
function renderDbEditor(){
  ...
  if(currentDbTab==="people"){
    ...
    box.innerHTML=`<div class="profile-top">${photoHtmlSync(p)}<div><h3>${escapeHtml(fullName(p))}</h3><p class="small">${faces.filter(f=>f.person_id===p.id).length} tagged photo${faces.filter(f=>f.person_id===p.id).length===1?'':'s'}</p></div></div>
      <div class="form-grid">
        <label>Full/current name<input id="editDisplayName" value="${escapeHtml(fullName(p))}" onblur="this.value=titleCaseName(this.value)"></label>
        <label>Preferred name<input id="editPreferred" value="${escapeHtml(p.preferred_name||p.given_names||'')}" onblur="this.value=titleCaseName(this.value)"></label>
        <label>Birth name<input id="editBirthName" value="${escapeHtml(p.birth_name||'')}" placeholder="e.g. Jean Davey" onblur="this.value=titleCaseName(this.value)"></label>
        <label>Family name<input id="editFamily" value="${escapeHtml(p.family_name||'')}" onblur="this.value=titleCaseName(this.value)"></label>
        ...
        <label>Occupation<input id="editOccupation" value="${escapeHtml(p.occupation||'')}"></label>
        <label>Biography<textarea id="editBio" placeholder="Short biographical note…">${escapeHtml(p.biography||'')}</textarea></label>
      </div>
    ...
    box.innerHTML=`<div class="form-grid">
      ...
      <label>Date accuracy<select id="editPhotoDateAccuracy"><option value="exact" ${ph.date_accuracy==='exact'?'selected':''}>Exact</option><option value="approx" ${ph.date_accuracy==='approx'?'selected':''}>Approximate</option><option value="year" ${ph.date_accuracy==='year'?'selected':''}>Year only</option><option value="unknown" ${ph.date_accuracy==='unknown'?'selected':''}>Unknown</option></select></label>
      <label>Photographer<input id="editPhotoPhotographer" value="${escapeHtml(ph.photographer||'')}"></label>
...
async function savePersonRecord(id){
  ...
  const patch={
    display_name:name,
    preferred_name:titleCaseName(el("editPreferred")?.value||name.split(" ")[0])||null,
    given_names:titleCaseName(el("editPreferred")?.value||name.split(" ")[0]),
    family_name:titleCaseName(el("editFamily")?.value||name.split(" ").slice(1).join(" "))||null,
    birth_name:titleCaseName(el("editBirthName")?.value||'')||null,
    birth_date:el("editBirth")?.value||null,
    occupation:el("editOccupation")?.value||null,
    biography:el("editBio")?.value||null
  };
...
async function savePhotoRecord(id){
  ...
  if('date_accuracy' in ph)patch.date_accuracy=el("editPhotoDateAccuracy")?.value||'exact';
  if('photographer' in ph)patch.photographer=el("editPhotoPhotographer")?.value||null;
...
Object.assign(window,{leavePhoto,setPhotoEditMode,addPhotoComment,suggestSelectedFaceName,suggestFaceForPhoto,newFeedbackPrompt,setSuggestionStatus,setFeedbackStatus,showDeathDateField,sendLogin,signOut,showPage,refreshData,uploadPhoto,selectLatestPhoto,addFaceBox,deleteSelectedFace,saveFaceName,setRel,saveRelationship,deleteRelationship,renderGraph,fitGraph,zoomGraph,resetGraphZoom,addUnknownPerson,deletePerson,toggleFaceBoxes,toggleFaceNames,applyTheme,titleCaseName,showDbTab,renderDatabase,selectDbPerson,selectDbPhoto,savePersonRecord,savePhotoRecord});
boot();
