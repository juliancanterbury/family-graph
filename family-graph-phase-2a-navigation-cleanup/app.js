...
function showPage(page){
  ...
  // Clear transient photo-editing state when navigating so the app never feels trapped.
  if(page !== "photo"){
    selectedFaceId = null;
    dragState = null;
    setEditMode(false);
  ...
  // Reset any scroll state that made the photo editor appear stuck.
  document.querySelectorAll('.page').forEach(p=>{ if(p!==target) p.scrollTop = 0; });

  if(page==="photo"){
    setEditMode(false); // photos always open in safe view mode
    selectedFaceId = null;
    renderCurrentPhoto().then(()=>{
      updateSide();
      const ps = el('photoScroll');
      const sc = document.querySelector('.photo-scroll');
...
window.addEventListener('keydown',ev=>{
  ...
  if(ev.key==='Escape'){
    selectedFaceId=null; setEditMode(false); renderCurrentPhoto(); renderFaceEditor();
  }
  if(location.hash==='#photo' && ev.key==='ArrowLeft') previousPhoto();
  if(location.hash==='#photo' && ev.key==='ArrowRight') nextPhoto();
});
...
async function uploadPhoto(ev){
  ...
  await renderCurrentPhoto(); updateSide(); renderPhotoListNav(); setStatus("Photo saved — detecting faces…");
...
async function selectLatestPhoto(){
  if(photos[0]) await selectPhoto(photos[0].id);
}
async function previousPhoto(){
  if(!photos.length)return;
  const i=Math.max(0, photos.findIndex(p=>p.id===currentPhoto?.id));
  const nextIndex=(i+1)%photos.length;
  await selectPhoto(photos[nextIndex].id);
}
async function nextPhoto(){
  if(!photos.length)return;
  let i=photos.findIndex(p=>p.id===currentPhoto?.id);
  if(i<0)i=0;
...
function renderFaceEditor(){
  ...
  w.innerHTML=`<div class="row"><label>Name</label><input id="faceName" value="${escapeHtml(p?fullName(p):(f.label||""))}" placeholder="Unnamed person" oninput="this.value=titleCaseName(this.value)"></div><button class="primary" onclick="saveFaceName()">Save name</button><button class="full" onclick="suggestSelectedFaceName()">Suggest correction instead</button>`;
}
...
async function renderPhotoListNav(){
  const box=el('photoListNav'); if(!box)return;
  if(!photos.length){box.innerHTML='<p class="small">No photos yet.</p>';return;}
  let html='';
  for(const ph of photos.slice(0,18)){
    const active = currentPhoto && ph.id===currentPhoto.id ? ' active' : '';
    const count = faces.filter(f=>f.photo_id===ph.id).length;
    const date = ph.date_taken || ph.photo_date || ph.taken_at || ph.created_at || '';
    let thumb='';
    try{ thumb = await photoUrl(ph); }catch(e){}
    html += `<button class="photo-list-item${active}" onclick="selectPhoto('${ph.id}')">
      <span class="photo-list-thumb" style="${thumb?`background-image:url('${thumb}')`:''}"></span>
      <span><b>${escapeHtml(photoTitle(ph))}</b><small>${escapeHtml(String(date).slice(0,10))}${count?` · ${count} faces`:''}</small></span>
    </button>`;
  }
  box.innerHTML=html;
}
