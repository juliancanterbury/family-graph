...
async function ensureProfile(){
  ...
  let found=await sb.from("profiles").select("*").eq("user_id",user.id).maybeSingle();
...
async function photoUrl(photo){if(!photo)return"";const bucket=typeof FAMILY_MEDIA_BUCKET!=="undefined"?FAMILY_MEDIA_BUCKET:"family-media";return sb.storage.from(bucket).getPublicUrl(photo.storage_path).data.publicUrl}
...
async function addFaceBox(){
  ...
  const ins=await sb.from("faces").insert({photo_id:currentPhoto.id,x:200,y:160,w:70,h:90,label:"Unnamed",created_by:session.user.id}).select().single();
  if(ins.error){alert(ins.error.message);return}
...
function renderFaceEditor(){
  ...
  w.innerHTML=`<div class="row"><label>Name</label><input id="faceName" value="${p?fullName(p):(f.label||"")}" placeholder="Full name or Unknown"></div><button class="primary" onclick="saveFaceName()">Save name</button>`;
}
async function saveFaceName(){
  const f=faces.find(x=>x.id===selectedFaceId), name=el("faceName")?.value.trim();
  if(!f||!name)return;
...
function updateSide(){
  ...
  const opts=people.map(p=>`<option value="${p.id}">${fullName(p)}</option>`).join("");safeHTML("relA",opts);safeHTML("relB",opts);renderFaceEditor();renderRelationshipList();previewRelationship();
}
async function addUnknownPerson(){
  const name=prompt("Name, or leave blank for Unknown");
  const ins=await sb.from("people").insert({display_name:name||"Unknown person",given_names:name||"Unknown",created_by:session.user.id}).select().single();
  if(ins.error){alert(ins.error.message);return}
  people.push(ins.data);updateDashboard();await renderPeople();updateSide();
}
async function renderPeople(){
  const list=el("peopleList");if(!list)return;let html="";
  for(const p of people){html+=`<div class="people-card">${await photoHtml(p)}<strong>${fullName(p)}</strong><p>${p.birth_date||"No dates yet"}</p></div>`}
...
function parentGroups(){const byChild={};relationships.filter(r=>r.relationship_type==="parent").forEach(r=>(byChild[r.to_person_id]||=[]).push(r.from_person_id));const groups={};Object.entries(byChild).forEach(([child,parents])=>{const key=parents.slice().sort().join("|");if(!groups[key])groups[key]={parents:parents.slice().sort(),children:[]};groups[key].children.push(child)});return Object.values(groups)}
function generationDepth(id,seen=new Set()){if(seen.has(id))return 0;seen.add(id);const ps=parentsOf(id);if(!ps.length)return 0;return 1+Math.max(...ps.map(p=>generationDepth(p,seen)))}
function layoutPositions(){
  const pos={},rows={};people.forEach(p=>{const d=generationDepth(p.id);(rows[d]||=[]).push(p.id)});
  Object.entries(rows).forEach(([d,ids])=>{ids.sort((a,b)=>fullName(person(a)).localeCompare(fullName(person(b))));ids.forEach((id,i)=>pos[id]={x:320+i*260,y:170+Number(d)*430})});
  relationships.filter(r=>r.relationship_type==="partner").forEach(r=>{if(pos[r.from_person_id]&&pos[r.to_person_id]){pos[r.to_person_id].y=pos[r.from_person_id].y;pos[r.to_person_id].x=pos[r.from_person_id].x+230}});
  parentGroups().forEach(g=>{const parents=g.parents.filter(id=>pos[id]),kids=g.children.filter(id=>pos[id]);if(!parents.length||!kids.length)return;const pc=parents.reduce((s,id)=>s+pos[id].x,0)/parents.length,start=pc-((kids.length-1)*260)/2;kids.forEach((id,i)=>pos[id].x=start+i*260)});
  return pos
}
async function renderGraph(){
  const graph=el("graph");if(!graph)return;graph.innerHTML='<div class="graph-inner" id="graphInner"></div>';const inner=el("graphInner"),pos=layoutPositions();
  relationships.filter(r=>r.relationship_type==="partner"||r.relationship_type==="sibling").forEach(r=>{if(!pos[r.from_person_id]||!pos[r.to_person_id])return;const y=pos[r.from_person_id].y+(r.relationship_type==="partner"?78:145);drawH(inner,pos[r.from_person_id].x+178,y,pos[r.to_person_id].x-(pos[r.from_person_id].x+178));label(inner,pos[r.from_person_id].x+185,y-25,r.relationship_type)});
  parentGroups().forEach(group=>{const parents=group.parents.filter(id=>pos[id]),children=group.children.filter(id=>pos[id]);if(!parents.length||!children.length)return;const pc=parents.reduce((s,id)=>s+pos[id].x+89,0)/parents.length,parentBottom=Math.max(...parents.map(id=>pos[id].y+156)),childTop=Math.min(...children.map(id=>pos[id].y)),busY=(parentBottom+childTop)/2;drawV(inner,pc,parentBottom,busY-parentBottom);const left=Math.min(...children.map(id=>pos[id].x+89)),right=Math.max(...children.map(id=>pos[id].x+89));drawH(inner,left,busY,right-left);children.forEach(id=>drawV(inner,pos[id].x+89,busY,pos[id].y-busY))});
  for(const p of people){const xy=pos[p.id]||{x:100,y:100},n=document.createElement("div");n.className="node";n.style.left=xy.x+"px";n.style.top=xy.y+"px";n.innerHTML=`${await photoHtml(p)}<strong>${fullName(p)}</strong><span class="small">${p.birth_date||""}</span>`;inner.appendChild(n)}
  applyGraphZoom()
}
...
Object.assign(window,{sendLogin,signOut,showPage,refreshData,uploadPhoto,selectLatestPhoto,addFaceBox,deleteSelectedFace,saveFaceName,setRel,saveRelationship,deleteRelationship,renderGraph,fitGraph,zoomGraph,resetGraphZoom,addUnknownPerson});
boot();
