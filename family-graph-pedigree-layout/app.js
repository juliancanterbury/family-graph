const KEY="familyGraphPedigreeLayout";
...
function load(){
  try{
    const v =
      ...
      localStorage.getItem("familyGraphSiblingSupport") ||
      localStorage.getItem("familyGraphMemoryFix") ||
      localStorage.getItem("familyGraphPhotoCardsZoom") ||
      localStorage.getItem("familyGraphValidationMvp") ||
      localStorage.getItem("familyGraphCleanPhotoMvp");
    return JSON.parse(v)||blank();
  }catch{return blank()}
}
...
function loadPhoto(ev){
  ...
  r.onload=e=>{
    if(data.faces.length&&!confirm("Changing to a different photo will clear the existing face boxes for this photo. Continue?")){
      ev.target.value="";return;
    }
    data.photoSrc=e.target.result;data.photoKind="uploaded";data.faces=[];selectedFaceId=null;save();renderPhoto();updateSide();
  };
...
function renderPhoto(){
  markSaved();
  document.getElementById("mainPhoto").src=data.photoSrc||"";
...
function addFaceBox(){
  ...
  data.faces.push(f);selectedFaceId=f.id;save();renderPhoto();renderFaceEditor();updateSide();
}
...
function deleteSelectedFace(){
  ...
  data.faces=data.faces.filter(f=>f.id!==selectedFaceId);selectedFaceId=null;save();renderPhoto();renderFaceEditor();updateSide();
}
function renderFaceEditor(){
  const wrap=document.getElementById("faceEditor"),f=data.faces.find(x=>x.id===selectedFaceId);
  if(!f){wrap.innerHTML="<p>Select or add a face box.</p>";return}
  const p=f.person_id?person(f.person_id):null;
  wrap.innerHTML=`<div class="row"><label>Name</label><input id="faceName" value="${p?fullName(p):""}" placeholder="Full name"></div><button class="primary" onclick="saveFaceName()">Save name</button><p class="small">This face crop becomes the temporary profile image.</p>`;
}
function saveFaceName(){
  const f=data.faces.find(x=>x.id===selectedFaceId),name=document.getElementById("faceName").value.trim();
  if(!f||!name)return;
  ...
  f.person_id=p.id;f.label=fullName(p);save();renderPhoto();renderFaceEditor();updateSide();renderPeople();
}

/* IMAGE CROPS */
function faceForPerson(id){return data.faces.find(f=>f.person_id===id)}
function cropStyle(f,size=92){
  if(!f||!data.photoSrc)return "";
  const scale=size/Math.max(f.w,f.h);
  const x=-(f.x*scale)+(size-f.w*scale)/2;
  const y=-(f.y*scale)+(size-f.h*scale)/2;
  return `background-image:url('${data.photoSrc}');background-size:${1200*scale}px auto;background-position:${x}px ${y}px;background-repeat:no-repeat;`
}
function photoHtml(p){
  const f=faceForPerson(p.id);
  if(f&&data.photoSrc)return `<div class="node-photo" style="${cropStyle(f)}"></div>`;
...
function setRel(type,btn){
  ...
  btn.classList.add("active");previewRelationship();
}
function relationshipCandidate(){
  ...
  else if(type==="partner"){type="partner";label="partner"}
...
function childrenOf(id){return data.relationships.filter(r=>r.type==="parent"&&r.from===id).map(r=>r.to)}
function partnersOf(id){return data.relationships.filter(r=>r.type==="partner"&&(r.from===id||r.to===id)).map(r=>r.from===id?r.to:r.from)}
function siblingsOf(id){return data.relationships.filter(r=>r.type==="sibling"&&(r.from===id||r.to===id)).map(r=>r.from===id?r.to:r.from)}
function isAncestor(ancestor,desc,seen=new Set()){
  if(seen.has(desc))return false;seen.add(desc);
  const ps=parentsOf(desc);if(ps.includes(ancestor))return true;
...
function validationFor(c){
  ...
  const dup=data.relationships.some(r=>r.from===c.from&&r.to===c.to&&r.type===c.type);
  const revPartner=data.relationships.some(r=>r.type==="partner"&&c.type==="partner"&&r.from===c.to&&r.to===c.from);
  const revSibling=data.relationships.some(r=>r.type==="sibling"&&c.type==="sibling"&&r.from===c.to&&r.to===c.from);
  if(dup||revPartner||revSibling)fatal.push("This relationship already exists.");
  if(c.type==="parent"){
    if(isAncestor(c.to,c.from))fatal.push("This would create a loop: someone would become their own ancestor.");
    const ps=data.relationships.filter(r=>r.type==="parent"&&r.to===c.to);
    if(ps.length>=2&&!ps.some(r=>r.from===c.from))issues.push("This person already has two parents recorded. Check carefully.");
    if(hasPartner(c.from,c.to))fatal.push("A partner cannot also be recorded as a parent of the same person.");
  }
  if(c.type==="partner"&&(isAncestor(c.from,c.to)||isAncestor(c.to,c.from)))fatal.push("A direct ancestor/descendant should not also be a partner.");
  if(c.type==="sibling"&&(isAncestor(c.from,c.to)||isAncestor(c.to,c.from)))fatal.push("A direct ancestor/descendant should not also be a sibling.");
  return{fatal,issues}
}
...
function saveRelationship(){
  ...
  if(v.issues.length&&!confirm(v.issues.join("\\n")+"\\n\\nSave anyway?"))return;
  data.relationships.push({id:uid("rel"),from:c.from,to:c.to,type:c.type,label:c.label});
  save();updateSide();alert("Relationship saved.");
}
function relationshipIssues(){
  const out=[];
  data.relationships.forEach(r=>{
    const v=validationFor({...r,raw:r.label});
    const msgs=[...v.fatal,...v.issues].filter(m=>!m.includes("already exists"));
    if(msgs.length)out.push({id:r.id,msgs});
  });
  return out;
}
function relationshipSentence(r){
  const a=person(r.from),b=person(r.to);if(!a||!b)return"Missing person";
  if(r.type==="parent")return`${fullName(a)} is ${r.label||"parent"} of ${fullName(b)}`;
  if(r.type==="partner")return`${fullName(a)} is partner of ${fullName(b)}`;
...
function updateRelationshipList(){
  ...
  document.getElementById("relationshipList").innerHTML=data.relationships.map(r=>{
    const issue=issues.find(i=>i.id===r.id);
    return`<div class="rel-row ${issue?'issue':''}"><div><strong>${relationshipSentence(r)}</strong>${issue?`<div class="meta">⚠ ${issue.msgs.join("; ")}</div>`:""}</div><button class="danger" onclick="deleteRelationship('${r.id}')">Delete</button></div>`
...
function updateSide(){
  ...
  renderFaceEditor();updateRelationshipList();previewRelationship();
}

/* PEDIGREE LAYOUT */
function familyKey(parents){return parents.slice().sort().join("|")}
function parentGroups(){
  const byChild={};
  data.relationships.filter(r=>r.type==="parent").forEach(r=>(byChild[r.to]||=[]).push(r.from));
  const groups={};
...
function collectDepths(){
  const depths={};
  data.people.forEach(p=>depths[p.id]=generationDepth(p.id));
  // siblings must share generation
  let changed=true;
  while(changed){
    changed=false;
    data.relationships.filter(r=>r.type==="sibling").forEach(r=>{
      const d=Math.min(depths[r.from]??0,depths[r.to]??0);
      if(depths[r.from]!==d){depths[r.from]=d;changed=true}
      if(depths[r.to]!==d){depths[r.to]=d;changed=true}
    });
  }
  // partners usually same generation, but never override parent/child chains
  data.relationships.filter(r=>r.type==="partner").forEach(r=>{
    if(!isAncestor(r.from,r.to)&&!isAncestor(r.to,r.from)){
      const d=Math.max(depths[r.from]??0,depths[r.to]??0);
      depths[r.from]=d;depths[r.to]=d;
    }
  });
  return depths;
}
function layoutPositions(){
  const depths=collectDepths();
  const pos={};
  const rows={};
  data.people.forEach(p=>(rows[depths[p.id]??0]||=[]).push(p.id));

  // rough ordering: keep couples near each other, then siblings.
  Object.entries(rows).forEach(([depth,ids])=>{
    ids.sort((a,b)=>{
      const pa=partnersOf(a)[0], pb=partnersOf(b)[0];
      if(pa===b)return -1;
      if(pb===a)return 1;
      return fullName(person(a)).localeCompare(fullName(person(b)));
    });
    ids.forEach((id,i)=>pos[id]={x:320+i*260,y:170+Number(depth)*430});
  });

  // Pull partners next to each other
  data.relationships.filter(r=>r.type==="partner").forEach(r=>{
    if(pos[r.from]&&pos[r.to]&&!isAncestor(r.from,r.to)&&!isAncestor(r.to,r.from)){
      pos[r.to].y=pos[r.from].y;
      pos[r.to].x=pos[r.from].x+230;
    }
  });

  // Pull siblings next to each other if same generation
  data.relationships.filter(r=>r.type==="sibling").forEach(r=>{
    if(pos[r.from]&&pos[r.to]){
      pos[r.to].y=pos[r.from].y;
      if(Math.abs(pos[r.to].x-pos[r.from].x)>520) pos[r.to].x=pos[r.from].x+260;
    }
  });

  // Centre children under their actual parent group only
  parentGroups().forEach(group=>{
    const parents=group.parents.filter(id=>pos[id]);
    const kids=group.children.filter(id=>pos[id]);
...
function renderGraph(){
  ...
  data.relationships.filter(r=>r.type==="partner").forEach(r=>{
    ...
    label(inner,pos[r.from].x+185,y-25,"partner");
  });

  // sibling lines
  data.relationships.filter(r=>r.type==="sibling").forEach(r=>{
    if(!pos[r.from]||!pos[r.to])return;
    const y=pos[r.from].y+145;
    drawH(inner,pos[r.from].x+178,y,pos[r.to].x-(pos[r.from].x+178));
    label(inner,pos[r.from].x+185,y-25,"sibling");
  });
  ...
  data.people.forEach(p=>{
    ...
    e.className="node";
    e.style.left=xy.x+"px";e.style.top=xy.y+"px";
