...
export async function renderPhotos(){
  ...
  const samePeopleOnly=document.querySelector('#photoFilterToggle [data-filter="same"]')?.classList.contains('primary');
  if(samePeopleOnly&&S.currentPhoto){
    const peopleHere=new Set(S.faces.filter(f=>f.photo_id===S.currentPhoto.id&&f.person_id).map(f=>f.person_id));
    if(peopleHere.size)list=list.filter(ph=>ph.id===S.currentPhoto.id||S.faces.some(f=>f.photo_id===ph.id&&f.person_id&&peopleHere.has(f.person_id)));
  }
  if(!list.length){box.innerHTML=samePeopleOnly?'<p class="small">No other photos with these people yet.</p>':'<p class="small">No photos in view yet — try "Show everyone" if you expected to see some here.</p>';return}
  let out=''; for(const ph of list){const url=await publicUrl(ph); const fc=S.faces.filter(f=>f.photo_id===ph.id).length; const nc=S.faces.filter(f=>f.photo_id===ph.id&&f.person_id).length; const d=ph.taken_date||ph.created_at?.slice(0,10)||''; out+=`<button class="photo-list-item ${S.currentPhoto?.id===ph.id?'active':''}" data-photo-id="${ph.id}"><img src="${url}"><b>${esc(d||'Undated')}${ph.private?' 🔒':''}</b>${ph.title?`<span class="photo-list-title">${esc(ph.title)}</span>`:''}<small>${fc} face${fc===1?'':'s'}${nc?` · ${nc} named`:''}</small></button>`}
  box.innerHTML=out;
}
...
export function renderFaceEditor(){
  ...
  w.innerHTML=`<div class="selected-face-summary"><p class="small">Current</p><strong>${p?`<button class="link-btn" data-person-id="${p.id}">${esc(fullName(p))} →</button>`:esc(f.label||'Unnamed face')}</strong></div>${suggestionHtml}<div class="form-grid compact-form"><label>Use existing person<div id="existingPersonSelect" class="person-picker custom-select" data-value="${f.person_id||''}">${personPickerHtml(f.person_id||'')}</div></label><button class="primary full" id="attachExistingBtn">Use selected person</button><label>Create new person<input id="faceName" value="${esc(p?fullName(p):(f.label||''))}" placeholder="Type a new full name"></label><button class="full" id="saveTypedFaceBtn">Create / save typed name</button><button class="full" id="suggestCorrectionBtn">Suggest correction instead</button></div>`;
}
...
export async function postComment(){if(!canEdit())return alert('You don\'t have permission to post comments.'); const body=$('commentText')?.value.trim(); if(!body)return; const row=await addOptional('comments',{photo_id:S.currentPhoto?.id,body,author_id:userId(),author_name:userName(),status:'open'}); S.comments.unshift(row); $('commentText').value=''; renderComments()}
...
export function bindPhotos(){document.body.addEventListener('click',e=>{const p=e.target.closest('[data-photo-id]'); if(p)selectPhoto(p.dataset.photoId)}); $('photoFilterToggle')?.addEventListener('click',e=>{const b=e.target.closest('[data-filter]'); if(!b)return; document.querySelectorAll('#photoFilterToggle [data-filter]').forEach(x=>x.classList.toggle('primary',x===b)); renderPhotos()}); $('photoInput')?.addEventListener('change',uploadPhoto); $('dashboardPhotoInput')?.addEventListener('change',uploadPhotoAndOpen); $('showUploadScreenBtn')?.addEventListener('click',()=>{S.currentPhoto=null;S.selectedFaceId=null;renderPhotoPage()}); $('latestPhotoBtn')?.addEventListener('click',latestPhoto); $('prevPhotoBtn')?.addEventListener('click',prevPhoto); $('nextPhotoBtn')?.addEventListener('click',nextPhoto); $('addFaceBtn')?.addEventListener('click',addFaceBox); $('detectFacesBtn')?.addEventListener('click',()=>detectFaces(false)); $('deleteFaceBtn')?.addEventListener('click',deleteFace); $('toggleBoxesBtn')?.addEventListener('click',()=>{S.showBoxes=!S.showBoxes;renderPhotoPage()}); $('toggleNamesBtn')?.addEventListener('click',()=>{S.showNames=!S.showNames;renderPhotoPage()}); $('suggestFaceBtn')?.addEventListener('click',suggestFace); $('savePhotoMetaBtn')?.addEventListener('click',savePhotoMeta); $('postCommentBtn')?.addEventListener('click',postComment); document.body.addEventListener('click',e=>{if(e.target.id==='attachExistingBtn')attachExisting(); if(e.target.id==='saveTypedFaceBtn')saveFaceName(); if(e.target.id==='suggestCorrectionBtn'||e.target.id==='suggestFaceInline')suggestFace(); const sug=e.target.closest('#confirmSuggestedFace'); if(sug)confirmSuggestedFace(sug.dataset.suggestedPerson)}); $('mainPhoto')?.addEventListener('click',()=>{if(!S.editMode){S.selectedFaceId=null;renderFaceEditor()}});
