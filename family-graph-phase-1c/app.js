...
let currentDbTab="people", selectedDbId=null;
  ...
  if(currentDbTab==="people"){
    ...
    box.innerHTML=`<div class="profile-top">${photoHtmlSync(p)}<div><h3>${escapeHtml(fullName(p))}</h3><p class="small">${faces.filter(f=>f.person_id===p.id).length} tagged photo${faces.filter(f=>f.person_id===p.id).length===1?'':'s'}</p></div></div>
      <div class="form-grid">
        ...
        <label>Death date<input id="editDeath" type="date" value="${escapeHtml(p.death_date||'')}"></label>
      </div>
      <button class="primary full" onclick="savePersonRecord('${p.id}')">Save person</button>
      <button class="danger full" onclick="deletePerson('${p.id}')">Delete person</button>`;
  }else if(currentDbTab==="photos"){
...
function photoHtmlSync(p){const f=faceForPerson(p.id);return f?`<div class="node-photo"></div>`:`<div class="node-photo">${initials(p)}</div>`}
async function savePersonRecord(id){
  ...
  const patch={display_name:name,given_names:titleCaseName(el("editGiven")?.value||name.split(" ")[0]),family_name:titleCaseName(el("editFamily")?.value||name.split(" ").slice(1).join(" "))||null,birth_date:el("editBirth")?.value||null,death_date:el("editDeath")?.value||null};
...
Object.assign(window,{sendLogin,signOut,showPage,refreshData,uploadPhoto,selectLatestPhoto,addFaceBox,deleteSelectedFace,saveFaceName,setRel,saveRelationship,deleteRelationship,renderGraph,fitGraph,zoomGraph,resetGraphZoom,addUnknownPerson,deletePerson,toggleFaceBoxes,toggleFaceNames,applyTheme,titleCaseName,showDbTab,renderDatabase,selectDbPerson,selectDbPhoto,savePersonRecord,savePhotoRecord});
boot();
