...
function ownerEmail(){ return "julian.canterbury@gmail.com"; }

async function boot(){
  try{
    hide("login"); hide("app"); hide("setupProblem"); show("loading");

    if(typeof SUPABASE_URL==="undefined" || typeof SUPABASE_ANON_KEY==="undefined" || !SUPABASE_URL || SUPABASE_URL.includes("PASTE_")){
      return showSetupProblem("config.js is missing or still contains placeholder values.");
    }
    ...
    console.error(err);
    showSetupProblem(err.message);
  }
}

function showSetupProblem(msg){
  hide("loading"); hide("login"); hide("app"); show("setupProblem");
  safeText("setupProblemText", msg);
}

async function route(){
  if(!session){
    hide("loading"); hide("app"); hide("setupProblem"); show("login");
    return;
  }

  hide("loading"); hide("login"); hide("setupProblem"); show("app");
  await ensureProfile();
  await loadAll();
  showPage("photo");
}
...
async function ensureProfile(){
  ...
  if(found.error && found.error.code !== "PGRST116"){
    console.warn(found.error);
  }

  if(found.data){
    currentProfile = found.data;
  }else{
    const profile = {
      user_id:user.id,
      email,
      display_name: email.split("@")[0],
      role: email.toLowerCase() === ownerEmail() ? "owner" : "contributor"
    };
...
async function loadAll(){
  ...
  if(err){
    showSetupProblem("Database read failed: " + err.message + ". You may not be signed in, or RLS policies need checking.");
    return;
  }
...
async function uploadPhoto(ev){
  ...
  const up = await sb.storage.from(FAMILY_MEDIA_BUCKET || "family-media").upload(path,file,{upsert:false});
...
async function endPointer(){
  if(dragState){
    ...
    if(f) await sb.from("faces").update({x:f.x,y:f.y,w:f.w,h:f.h}).eq("id",f.id);
  }
  dragState=null;
...
function relationshipCandidate(){
  const a=el("relA")?.value, b=el("relB")?.value;
...
function isAncestor(a,d,seen=new Set()){
  if(seen.has(d)) return false;
  seen.add(d);
  const ps=parentsOf(d);
  if(ps.includes(a)) return true;
  return ps.some(p=>isAncestor(a,p,seen));
}
function validationFor(c){
  ...
  if(["partner","sibling"].includes(c.type)&&(isAncestor(c.from,c.to)||isAncestor(c.to,c.from))) fatal.push("This conflicts with a parent/child chain.");
  return {fatal,issues:[]};
}
function previewRelationship(){
  const box=el("relationshipWarning"); if(!box) return;
...
async function saveRelationship(){
  ...
  const ins=await sb.from("relationships").insert({
    from_person_id:c.from,
    to_person_id:c.to,
    relationship_type:c.type,
    label:c.label,
    source_photo_id:currentPhoto?currentPhoto.id:null,
    created_by:session.user.id
  }).select().single();
  if(ins.error){alert(ins.error.message); return;}
...
function renderRelationshipList(){
  safeHTML("relationshipList",relationships.map(r=>`<div class="rel-row"><div><strong>${relationshipSentence(r)}</strong></div><button class="danger" onclick="deleteRelationship('${r.id}')">Delete</button></div>`).join("")||"<p>No relationships yet.</p>");
}
function updateSide(){
  ...
  safeHTML("relA",opts); safeHTML("relB",opts);
...
async function renderGraph(){
  ...
  const inner=el("graphInner"), pos=layoutPositions();
...
function drawH(g,x,y,w){
  const e=document.createElement("div"); e.className="line h";
  if(w<0){e.style.left=x+w+"px"; e.style.width=-w+"px";}else{e.style.left=x+"px"; e.style.width=w+"px";}
  e.style.top=y+"px"; g.appendChild(e);
}
function drawV(g,x,y,h){
  const e=document.createElement("div"); e.className="line v";
  e.style.left=x+"px"; e.style.top=y+"px"; e.style.height=Math.max(0,h)+"px"; g.appendChild(e);
}
function label(g,x,y,text){
  const e=document.createElement("div"); e.className="rel-label";
  e.style.left=x+"px"; e.style.top=y+"px"; e.textContent=text; g.appendChild(e);
}
function applyGraphZoom(){
  const inner=el("graphInner"); if(inner) inner.style.transform=`scale(${graphScale})`;
  safeText("zoomLabel",Math.round(graphScale*100)+"%");
}
function zoomGraph(delta){
  const wrap=el("graphWrap"); if(!wrap) return;
  const old=graphScale, cx=wrap.scrollLeft+wrap.clientWidth/2, cy=wrap.scrollTop+wrap.clientHeight/2;
