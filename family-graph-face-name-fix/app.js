...
function titleCaseNameLive(v){
  const str=String(v||"");
  return str.split(/(\s+)/).map(tok=>{
    if(/^\s+$/.test(tok))return tok;
    return tok.split("-").map(piece=>piece?piece[0].toUpperCase()+piece.slice(1).toLowerCase():piece).join("-");
  }).join("");
}
function formatNameInput(input){
  if(!input)return;
  const start=input.selectionStart, end=input.selectionEnd;
  input.value=titleCaseNameLive(input.value);
...
function renderFaceEditor(){
  ...
  const sortedPeople=visiblePeople().sort((a,b)=>fullName(a).localeCompare(fullName(b)));
  const opts=['<option value="">Choose existing person…</option>'].concat(
    sortedPeople.map(pp=>`<option value="${pp.id}" ${pp.id===f.person_id?'selected':''}>${escapeHtml(fullName(pp))}</option>`)
  ).join('');
  const list=sortedPeople.map(pp=>`<option value="${escapeHtml(fullName(pp))}"></option>`).join('');
  w.innerHTML=`
    ...
    <div class="form-grid compact-form">
      <label>Quick name / search
        <input id="faceName" list="peopleNameList" value="${escapeHtml(p?fullName(p):(f.label||''))}" placeholder="Start typing a name" oninput="formatNameInput(this)" onblur="this.value=titleCaseName(this.value)">
        <datalist id="peopleNameList">${list}</datalist>
      </label>
      <button class="primary full" onclick="saveFaceName()">Save name</button>
      <label>Or choose existing person
        <select id="existingPersonSelect">${opts}</select>
      </label>
      <button class="full" onclick="attachFaceToExisting()">Use selected person</button>
